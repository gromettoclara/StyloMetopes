...
if "%doast%" == "oui" pandoc --standalone --verbose --citeproc ^
  -f commonmark_x+sourcepos+raw_html -t xml --metadata=link-citations:true ^
...
if "%dolat%" == "oui" pandoc --standalone --verbose ^
  -f commonmark_x+sourcepos -t html4 --mathml -o %dossout%\%fich%.mathml.xml ^
...
if "%dohtml%" == "oui" pandoc --standalone --verbose --section-divs ^
  ...
  -f commonmark_x+sourcepos+tex_math_dollars+raw_html -t html --mathml ^
  --bibliography %dossin%\%fich%.bib ^
  --csl %dossaux%\chicago-author-date-YMA.csl ^
  %dossin%\%fich%.yaml ^
